(function () {
  ...
  function computeMatchSummary(match, rounde) {
  ...
  function renderMatchesAccordion(root, rounde, matches, teams) {
    if (!root) return;
    var needed = roundsNeededForRounde(rounde);

    var sf1 = computeMatchSummary(matches[0], rounde);
    var sf2 = computeMatchSummary(matches[1], rounde);
    ...
    var matchMeta = [
      ...
      { label: "Final", phaseLine: "Fase · Final", left: sf1WinnerName, right: sf2WinnerName },
    ];
    ...
    var html = [];
    for (var i = 0; i < 3; i++) {
      var msum = computeMatchSummary(matches[i], rounde);
  ...
  function computeChampion(teams, matches, rounde) {
    var sf1 = computeMatchSummary(matches[0], rounde);
    var sf2 = computeMatchSummary(matches[1], rounde);
    var final = computeMatchSummary(matches[2], rounde);
  ...
  function buildBracketPosterHtml(teams, rounde, matches) {
    ...
    var champion = computeChampion(teams, matches, rounde) || "—";
    var championChip = badgeHtml("Champion: " + champion, champion && champion !== "—" ? "win" : "neutral");

    // Poster: SF kiri/kanan = round menang di semi; tengah = finalis + skor final (alur 4 → 2 → 1).
    var needed = roundsNeededForRounde(rounde);
    var s0 = computeMatchSummary(matches[0], rounde);
    var s1 = computeMatchSummary(matches[1], rounde);
    var fin = computeMatchSummary(matches[2], rounde);
    ...
    // Rel setinggi dua kartu saja (judul + badge di luar) → titik gabung 50% = tengah vertikal pasangan match.
    var bracketRailLeft =
    ...
    var bracketRailRight =
      '<div class="relative z-0 hidden w-[2.35rem] shrink-0 self-stretch lg:block" aria-hidden="true">' +
      '<svg class="absolute inset-0 h-full w-full overflow-visible text-[var(--color-brand-glow)]" preserveAspectRatio="none" viewBox="0 0 34 100">' +
    ...
    var bracket =
      ...
      '<p class="relative z-[1] m-0 mb-4 text-center font-[family-name:var(--font-heading)] text-[0.55rem] font-bold uppercase tracking-[0.24em] text-[color-mix(in_srgb,var(--color-text-secondary)_88%,var(--color-brand-glow))] lg:hidden">Single elimination · 4 → 2 → 1</p>' +
  ...
  function certArticleChampion(opts) {
    ...
    return (
      ...
      '<span class="br-m1-cert__aside-tag">Clash Squad · 4 Team</span></div></aside>' +
      ...
      '<p class="br-m1-cert__thanks">Clash Squad 4 Team · Rounde ' +
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    var matches = normalizedMatches(3, currentRounde(), state.matches);
    ...
    function recomputeAndPaint() {
      matches = normalizedMatches(3, currentRounde(), matches);
      var t = teamsNow();
      renderMatchesAccordion(matchesRoot, currentRounde(), matches, t);
      syncMetaLabels();

      var champ = computeChampion(t, matches, currentRounde());
      if (championBadge) {
        if (!champ) {
    ...
    form.addEventListener("change", function (e) {
      var t = e.target;
      if (!t) return;
      if (t.name === "rounde") recomputeAndPaint();
    });
    ...
    if (btnReset) {
      btnReset.addEventListener("click", function () {
        ...
        team3El.value = "";
        team4El.value = "";
        matches = normalizedMatches(3, currentRounde(), []);
        recomputeAndPaint();
      });
    }
    ...
    function renderBracketPosterIntoModal() {
      ...
      bracketCapture.innerHTML = buildBracketPosterHtml(teamsNow(), currentRounde(), matches);
    }
    ...
    function renderCertsIntoModal() {
      ...
      var fin = computeMatchSummary(matches[2], currentRounde());
      var totalLine = fin.roundWinsLeft + " - " + fin.roundWinsRight;
    ...
    if (certCapture) {
      certCapture.addEventListener("click", function (e) {
        ...
        var doDl = function () {
          ...
          var tourney = safeFilenamePart(textById("br-match1-tournament") || "FT_Epep_Squad");
          var date = safeFilenamePart(textById("br-match1-playdate") || "");
          var file = "CS4_Certificate_" + (tourney ? tourney : "Champion") + (date ? "_" + date : "") + ".png";
...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function renderCertsIntoModal() {
      ...
      // total skor dari semua match (round wins)
      var totalLeft = 0;
      var totalRight = 0;
